...
| Prop | 型別 | 預設 | 說明 |
| :--- | :--- | :---: | :--- |
| `zIndex` | `number` | `var(--cml-z-popup)` | 彈出層 z-index；未指定時採用[疊層刻度](../layering.md)的 popup 層級。以 `??` 判空，故傳入 `0` 視為有效值 |
...
- 預設層級高於 Drawer 與 BottomSheet（見[疊層刻度](../layering.md)），因為 Teleport 進 `<dialog>` 後三者互為同層兄弟節點。
