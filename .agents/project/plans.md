...
| [2606081005-aqua-complete-and-tailwind-refactor](./plans/2606081005-aqua-complete-and-tailwind-refactor/plan.md) | In Progress | [Button, Input, Switch, Checkbox, Tabs, Dialog, BottomSheet, Steps, Skeleton, Toast, Loading, SelectV2, +Aqua/*, tailwind.css] | 為所有可主題化元件補完 Aqua 風格（pill/Spring/color-mix），並將 computed style 與 scoped CSS 全面重構為 Tailwind v4（共用 keyframes/animation 集中至 tailwind.css）；取代失效的 2506051810-aqua-pill-theme（已 commit + push 檢查點 e45c22e） |
| [2606081316-drawer-tree-table-components](./plans/2606081316-drawer-tree-table-components/plan.md) | Planning | [Drawer, Tree, TreeNode, Table, shared/types] | 新增 Drawer（左右/floating+fixed）、Tree（多階層+勾選連動）、Table（fixed header/columns/pinned rows、雙色/hover、cell/head slot、泛型 data），皆支援四風格主題 |
